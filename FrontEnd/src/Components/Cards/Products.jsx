...
const Products = () => {
  ...
  const getProduct = async (id_producto) => {
      let response = await axios.get(`${URL_PRODUCTOS}/${id_producto}`)
      console.log(response.data)
  ...
  const listarProductos = (array) => {
    ...
    let contenedor = document.getElementById('contenedorAside')
  ...
  return (
    ...
    {productos.map((item) => (
      ...
      <div className="card-burger mt-5" key={item.id_producto}>
    ...
    <div className='card__img'><img className="card__img" src={item.imagenProducto} alt="" /></div>
    ...
    <div className="card__wrapper">
        ...
        <div className="card__counter">
            <button onClick={() => handleClick(getProduct(item.id_producto))} className="card__counter-score text-decoration-none text-dark">Agregar Producto</button>
        </div>
    </div>
</div>
    ))
