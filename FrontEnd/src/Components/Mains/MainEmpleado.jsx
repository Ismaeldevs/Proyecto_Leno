...
const MainEmpleado = () => {
  ...
  const filtrarDatos = (terminoBusqueda) => {
    const busqueda = data.filter((datum) =>
     datum.cuilEmpleado.toLowerCase().trim().includes(terminoBusqueda) ||
     datum.nombreEmpleado.toLowerCase().trim().includes(terminoBusqueda) ||
     datum.apellidoEmpleado.toLowerCase().trim().includes(terminoBusqueda) 
    );
  ...
  return (
    <div>
      ...
      <Link to={`/empleados/create`} className="btn text-white bg-success m-5">
        CREAR EMPLEADO
      </Link>
        ...
        <tbody>
          {dataFiltrada.map((empleado) => (
            <tr key={empleado.id_Empleado}>
              ...
              <td>
                {
                  <Link to={`/empleados/view/${empleado.id_Empleado}`}>
                    <i className="me-3 text-primary fa-solid fa-eye"></i>
                  </Link>
                }
                {
                  <Link to={`/empleados/edit/${empleado.id_Empleado}`}>
                    <i className="me-3 text-warning fa-solid fa-pen-to-square"></i>
                  </Link>
                }
                {
                  <button onClick={() => handleChange(empleado.id_Empleado)}>
