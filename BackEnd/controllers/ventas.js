...
const editVentas = (req,res) =>{
    const {id_Producto,id_Cliente,id_Empleado,fechaVenta,cantidadVenta,descuentoVenta,tipoPagoVenta,totalVenta} = req.body
    const id = req.params.id
    const query = `update Ventas set id_Producto=${id_Producto},id_Cliente=${id_Cliente},id_Empleado=${id_Empleado},fechaVenta="${fechaVenta}",cantidadVenta=${cantidadVenta},descuentoVenta=${descuentoVenta},tipoPagoVenta="${tipoPagoVenta}",totalVenta=${totalVenta} where id_Venta=${id}`
 conection.query(query,(err,results)=>{
    if(err) throw err
    res.send(results)
 })
